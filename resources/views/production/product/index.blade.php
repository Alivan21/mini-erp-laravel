<x-app-layout>
  <x-slot name="header">
    <div class="flex justify-between items-center">
      ...
      <a href="{{ route('production.product.print_pdf') }}" class="no-underline">
        <button type="button"
        ...
        <div
          ...
          <div class="flex flex-col">
            ...
            <p class="mt-1 max-w-2xl text-sm leading-5 text-gray-500">
              Daftar barang yang diproduksi oleh perusahaan.
            </p>
          </div>
          <div class="flex items-center">
            <a href="{{ route('production.product.create') }}" class="no-underline">
              <button type="button"
                class="focus:shadow-outline-blue inline-flex items-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium leading-5 text-white transition duration-150 ease-in-out hover:bg-indigo-500 focus:border-blue-700 focus:outline-none active:bg-blue-700">
                <i class="fa-solid fa-circle-plus -ml-1 mr-2 text-lg"></i>
                <span>Tambah Barang Produksi</span>
              </button>
            </a>
          </div>
        </div>
        <form action="{{ route('production.product.index') }}" method="GET">
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
          <table class="w-full text-left text-sm text-gray-500 rtl:text-right">
            <thead class="bg-gray-50 text-xs uppercase text-gray-700">
              <tr>
                ...
                <th scope="col" class="px-6 py-3">Stok Barang</th>
              ...
              @foreach ($products as $product)
                <tr class="border-b bg-white hover:bg-gray-50">
                  <th scope="row" class="whitespace-nowrap px-6 py-4 font-medium text-gray-900">
                    {{ \Carbon\Carbon::parse($product->date)->translatedFormat('d F Y') }}
                  </th>
                  ...
                  <td class="whitespace-nowrap px-6 py-4">
                    <a href="{{ route('production.product.edit', $product->id) }}" class="no-underline">
                      <button type="button"
          ...
          <nav class="flex items-center justify-between p-4 border-t" aria-label="Table navigation">
            <span class="text-sm font-normal text-gray-500">
              ...
              of <span class="font-semibold text-gray-900">{{ $products->total() }}</span>
            </span>
            {{ $products->links() }}
          </nav>
        </div>
